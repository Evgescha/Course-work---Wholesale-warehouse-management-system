...
public class OrderFrame extends JFrame {
					...
					.addGroup(gl_panel_1.createParallelGroup(Alignment.LEADING)
						...
						.addGroup(Alignment.TRAILING, gl_panel_1.createSequentialGroup()
							.addGroup(gl_panel_1.createParallelGroup(Alignment.TRAILING)
								...
								.addComponent(dateChooser, Alignment.LEADING, GroupLayout.DEFAULT_SIZE, 142, Short.MAX_VALUE)
								.addComponent(textField_1, GroupLayout.DEFAULT_SIZE, 142, Short.MAX_VALUE))
							.addGap(26))
						.addGroup(gl_panel_1.createSequentialGroup()
							.addComponent(comboBox_2, 0, 142, Short.MAX_VALUE)
							.addGap(26))))
		);
	...
	private void actionTableMouseClicked() {
		if (table.getRowCount() > 0)
			try {
				...
				comboBox_2.getModel().setSelectedItem(order.getProduct());
	...
	private void actionUpdateButton() {
		...
		int row = table.getSelectedRow();
		Long id = Long.parseLong(table.getModel().getValueAt(row, OrderTableModal.ID_COL).toString());
		id_employee= ((Employee) (comboBox.getSelectedItem())).getId();
		id_client = ((Client) (comboBox_1.getSelectedItem())).getId();
		id_product = ((Warehouse) (comboBox_2.getSelectedItem())).getProduct().getId();
	...
	public void refreshComboBoxes() {
		try {
			...
			comboBox_2.setModel(
					new DefaultComboBoxModel(ApplicationController.warehouseController.getDAO().readAllWithCountMoreThenZero().toArray()));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
